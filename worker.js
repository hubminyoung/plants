...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      else if (pathname === '/api/mbg/search')       data = await mbgSearch(searchParams, env);
      else if (pathname === '/api/mbg/save-taxon')   data = await mbgSaveTaxon(req, env);
      else if (pathname === '/api/mbg/details')      data = await mbgDetails(searchParams, env);
...
// MBG 기준 신명→현재 accepted name (구→신)
...
// RHS/GSM/NDB 기준 신명→구명 (이 DB들은 구명 사용)
const GENUS_OLD_NAMES = {
  'eriocapitella': 'anemone',       // APG IV 2016
  'pennisetum':    'cenchrus',      // APG IV (일부 DB는 반대)
  'calamintha':    'clinopodium',   // 일부 DB
  'schizachyrium': 'andropogon',    // 일부 DB
  'symphyotrichum':'aster',         // 유럽 DB (RHS)
  'eutrochium':    'eupatorium',    // 유럽 DB
  'hylotelephium': 'sedum',         // 유럽 DB
  'actaea':        'cimicifuga',    // 일부 유럽 DB
};

async function mbgSearch(params, env) {
  ...
  try {
    ...
    // GENUS_OLD_NAMES fallback: JSON이 구명(Anemone)으로 등록된 경우 (예: Eriocapitella→Anemone)
    if (!githubTaxonId) {
      const genusWord = (base.split(' ')[0] || q.split(' ')[0]).toLowerCase();
      const oldGen = GENUS_OLD_NAMES[genusWord];
      if (oldGen) {
        const speciesPart = base.split(' ').slice(1).join(' ');
        const oldCandidates = speciesPart
          ? [toKey(oldGen + ' ' + speciesPart), toKey(oldGen)]
          : [toKey(oldGen)];
        for (const c of oldCandidates) {
          const tid = taxonMap.get(c);
          if (tid) { githubTaxonId = tid; githubMatchedName = c; break; }
        }
      }
    }
    // GENUS_SYNONYMS fallback: JSON이 신명(Actaea)으로 등록된 경우 (예: Cimicifuga→Actaea)
    if (!githubTaxonId) {
      const genusWord = (base.split(' ')[0] || q.split(' ')[0]).toLowerCase();
      const newGen = GENUS_SYNONYMS[genusWord];
      if (newGen) {
        const speciesPart = base.split(' ').slice(1).join(' ');
        const newCandidates = speciesPart
          ? [toKey(newGen + ' ' + speciesPart), toKey(newGen)]
          : [toKey(newGen)];
...
async function mbgKvSave(req, env) {
  if (!env?.PLANT_DATA) return { error: 'KV not bound' };
  let body;
  try { body = await req.json(); } catch(_) { return { error: 'invalid body' }; }
...
async function mbgSaveTaxon(req, env) {
  if (!env?.PLANT_DATA) return { error: 'KV not bound' };
  let items = [];
  try { const b = await req.json(); items = Array.isArray(b) ? b : [b]; } catch(_) { return { error: 'invalid body' }; }
  let saved = 0;
  for (const { name, taxonid } of items) {
    if (!name || !taxonid) continue;
    const key = 'mbg_taxon_' + name.toLowerCase().replace(/[''ʼ′]/g, "'").replace(/\s+/g, ' ').trim();
    try { await env.PLANT_DATA.put(key, String(taxonid)); saved++; } catch(_) {}
  }
...
async function mbgDetails(params, env) {
  ...
  // KV 캐시 확인 (Chrome에서 스크래핑한 데이터 우선 반환)
  if (env?.PLANT_DATA) {
    try {
      const cached = await env.PLANT_DATA.get('mbg_detail_' + taxonid, { type: 'json' });
      if (cached && (cached.culture || cached.noteworthy || cached.commonName)) {
...
async function gaissmayerDetails(params) {
  ...
  // 속명 동의어 후보 추가 (신명→구명, GSM은 구명 사용)
  const genusWord = baseNoVar.split(' ')[0].toLowerCase();
  const oldGenusG = GENUS_OLD_NAMES[genusWord];
  const synonymCandidates = oldGenusG
    ? [baseNoKeyword, baseNoVar, base].map(c => c.replace(new RegExp('^' + genusWord, 'i'), oldGenusG))
    : [];
  const searchCandidates = [...new Set([q, baseNoKeyword, baseNoVar, base, ...synonymCandidates].map(cleanQ))];
...
// -- RHS URL search via genus-indexed slug map cached in KV --
const RHS_KV_KEY = 'rhs_slug_map_v1';
let _rhsMapCache = null;

async function getRhsMap(env) {
  if (_rhsMapCache) return _rhsMapCache;
  if (!env || !env.PLANT_DATA) return null;
  try {
    const data = await env.PLANT_DATA.get(RHS_KV_KEY, { type: 'json' });
...
async function rhsBuildIndex(env) {
  if (!env || !env.PLANT_DATA) return { error: 'KV not bound' };
  const sitemapUrls = [1,2,3,4,5,6,7].map(n => `https://www.rhs.org.uk/sitemap-plants-${n}.xml`);
  const results = await Promise.all(sitemapUrls.map(async url => {
    try {
      const r = await fetch(url);
      if (!r.ok) return [];
      const xml = await r.text();
      return [...xml.matchAll(/\/plants\/(\d+)\/([a-z0-9-]+)\/details/g)].map(m => [m[2], parseInt(m[1])]);
    } catch(e) { return []; }
  }));
  const genusMap = {};
  for (const entries of results) {
    for (const [slug, id] of entries) {
      ...
      if (dash < 0) continue;
      const genus = slug.slice(0, dash);
      const rest = slug.slice(dash + 1);
      if (!genusMap[genus]) genusMap[genus] = {};
      genusMap[genus][rest] = id;
    }
  }
  const total = Object.values(genusMap).reduce((s, v) => s + Object.keys(v).length, 0);
  if (total > 0) {
    await env.PLANT_DATA.put(RHS_KV_KEY, JSON.stringify(genusMap));
    _rhsMapCache = genusMap;
  }
  return { ok: true, genera: Object.keys(genusMap).length, total };
}

async function rhsSearch(params, env) {
  const q = (params.get('q') ?? '').trim();
  if (!q) return { rhsUrl: null };

  const map = await getRhsMap(env);
  if (!map) return { rhsUrl: null };

  const words = q.toLowerCase().replace(/[.]/g, '').split(/\s+/);
  const genus = words[0];

  // Extract cultivar (in any type of quote)
  const cultivarMatch = q.match(/['‘’ʼ′]([^'‘’ʼ′]+)['‘’ʼ′]/);
  const cultivarSlug = cultivarMatch
    ? cultivarMatch[1].toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
    : null;

  // Base species parts (no cultivar, no var/subsp keywords)
  const noCultivar = q.replace(/['‘’ʼ′][^'‘’ʼ′]+['‘’ʼ′]/g, '').trim();
  const baseParts = noCultivar.toLowerCase().replace(/[.]/g, '').split(/\s+/).slice(1)
    .filter(w => !/^(var|subsp|f|ssp|cv)$/.test(w));
  const speciesRest = baseParts.join('-');

  const buildUrl = (g, rest, id) => `https://www.rhs.org.uk/plants/${id}/${g}-${rest}/details`;

  // Try current genus, then GENUS_OLD_NAMES synonym
  // NOTE: do NOT early-return when genus not found — synonym may still match (e.g. Eriocapitella→Anemone)
  const oldGenus = GENUS_OLD_NAMES[genus];
  const generaToTry = (oldGenus && oldGenus !== genus) ? [genus, oldGenus] : [genus];

  for (const tryGenus of generaToTry) {
    const entries = map[tryGenus];
    if (!entries) continue;

    if (cultivarSlug) {
      const exactRest = [...baseParts, cultivarSlug].join('-');
      if (entries[exactRest]) return { rhsUrl: buildUrl(tryGenus, exactRest, entries[exactRest]) };
      for (const [rest, id] of Object.entries(entries)) {
        if (rest.includes(cultivarSlug) && baseParts.every(p => rest.includes(p))) {
          return { rhsUrl: buildUrl(tryGenus, rest, id) };
        }
      }
    }

    if (speciesRest && entries[speciesRest]) {
      return { rhsUrl: buildUrl(tryGenus, speciesRest, entries[speciesRest]) };
    }

    // Fuzzy: prefix/suffix match for subsp./var. variations
    if (speciesRest) {
      for (const [rest, id] of Object.entries(entries)) {
        if (rest.startsWith(speciesRest + '-') || speciesRest.startsWith(rest + '-')) {
          return { rhsUrl: buildUrl(tryGenus, rest, id) };
        }
      }
    }
...
// ── RHS 상세 재배 정보 스크래핑 ────────────────────────────────────────────────
async function rhsDetails(params) {
  const url = (params.get('url') ?? '').trim();
  if (!url || !url.startsWith('https://www.rhs.org.uk/plants/')) return { error: 'invalid url' };
  ...
  function stripTags(s) {
    return s.replace(/<a[^>]*>([^<]*)<\/a>/g,'$1').replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim();
  }

  // h3 섹션 추출 (p 태그)
  function section(heading) {
    const re = new RegExp('<h3[^>]*>\\s*' + heading + '[^<]*<\\/h3>\\s*<p[^>]*>([\\s\\S]*?)<\\/p>', 'i');
    const m = html.match(re);
    return m ? ent(stripTags(m[1])).slice(0, 800) : '';
  }

  // h3 섹션 추출 (ul 리스트)
  function sectionList(heading) {
    const re = new RegExp('<h3[^>]*>\\s*' + heading + '[^<]*<\\/h3>[\\s\\S]*?<ul[^>]*>([\\s\\S]*?)<\\/ul>', 'i');
    const m = html.match(re);
    if (!m) return '';
    const items = [...m[1].matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)].map(x => ent(stripTags(x[1])));
    return items.join(', ');
  }

  // h2+p 카드형 속성 (Max Height, Max Spread 등)
  function card(label) {
    const re = new RegExp('>' + label + '<\/h2>[\\s\\S]{0,400}?<p[^>]*>([^<]+)<\/p>', 'i');
    const m = html.match(re);
    return m ? ent(m[1]) : '';
  }
  ...
  function dtdd(label) {
    ...
    const m = html.match(re);
    return m ? ent(stripTags(m[1])) : '';
  }

  // Hardiness H값 추출 (H1a~H7)
  const hardM = html.match(/>\s*(H[1-7][ab]?)\s*<\/dd>/i);
  const rhsHardiness = hardM ? hardM[1] : '';

  // Aspect → 간략화
  const rhsAspect = dtdd('Aspect');

  return {
    rhsCultivation:  section('Cultivation'),
    rhsPropagation:  section('Propagation'),
    rhsPruning:      section('Pruning'),
    rhsPests:        section('Pests'),
    rhsDiseases:     section('Diseases'),
    rhsLocations:    sectionList('Suggested planting'),
    // 기본 속성 (MBG 대체)
    rhsMaxHeight:    card('Max Height'),
    rhsMaxSpread:    card('Max Spread'),
    rhsHardiness,
    rhsMoisture:     dtdd('Moisture'),
    rhsAspect,
    rhsFamily:       dtdd('Family'),
    rhsHabit:        dtdd('Habit'),
    rhsRange:        dtdd('Plant Range'),
    rhsFoliage:      dtdd('Foliage'),
    rhsSoilTypes:    dtdd('Soil') || card('Soil Types'),
    fromRhs: true,
  };
}
...
// ── MBG 큐 관리 ────────────────────────────────────────────────────────────────
const MBG_QUEUE_KEY = 'mbg_pending_queue';

async function mbgQueueGet(env) {
  if (!env?.PLANT_DATA) return { error: 'KV not bound', queue: [] };
  try {
    const q = await env.PLANT_DATA.get(MBG_QUEUE_KEY, { type: 'json' });
    return { queue: q || [] };
  } catch(_) { return { queue: [] }; }
}

async function mbgQueueAdd(req, env) {
  if (!env?.PLANT_DATA) return { error: 'KV not bound' };
  let items = [];
  try { const b = await req.json(); items = Array.isArray(b) ? b : [b]; } catch(_) { return { error: 'invalid body' }; }
  const cur = await env.PLANT_DATA.get(MBG_QUEUE_KEY, { type: 'json' }) || [];
  for (const item of items) {
    if (!item.taxonid || item.taxonid.startsWith('static:')) continue;
    if (!cur.find(x => x.taxonid === item.taxonid)) cur.push({ taxonid: item.taxonid, name: item.name || '' });
  }
  await env.PLANT_DATA.put(MBG_QUEUE_KEY, JSON.stringify(cur));
  return { ok: true, queued: cur.length };
}

async function mbgQueueClear(env) {
  if (!env?.PLANT_DATA) return { error: 'KV not bound' };
  await env.PLANT_DATA.put(MBG_QUEUE_KEY, JSON.stringify([]));
  return { ok: true };
}

// ── MBG 배치 스크래핑 스크립트 생성 ────────────────────────────────────────────
// GET /api/mbg/batch-script?taxonids=123,456 (또는 POST로 list 전달)
// MBG 탭에서 직접 실행 가능한 JS 반환 — Worker IP 차단 우회
async function mbgBatchScript(params, env, req) {
  // taxonid 목록 수집: URL 파라미터 or POST body
  let taxonids = [];
  const paramList = params.get('taxonids');
  if (paramList) {
    taxonids = paramList.split(',').map(s => s.trim()).filter(Boolean);
  } else if (req.method === 'POST') {
    try { const b = await req.json(); taxonids = b.taxonids || []; } catch(_) {}
  }

  const workerUrl = 'https://plantlist.younrake.workers.dev';

  const script = `
// ══════════════════════════════════════════════════════
// MBG PlantFinder 배치 스크래핑 스크립트
// plantfinder.mobot.org 탭에서 콘솔에 붙여넣기 실행
// ══════════════════════════════════════════════════════
(async function mbgBatchEnrich() {
  const WORKER = '${workerUrl}';
  const TAXONIDS = ${JSON.stringify(taxonids)};

  function ent(s){ return s.replace(/&amp;/g,'&').replace(/&nbsp;/g,' ').replace(/&#\\d+;/g,'').replace(/&[a-z]+;/g,'').trim(); }
  function stripTags(s){ return s.replace(/<[^>]+>/g,' ').replace(/\\s+/g,' ').trim(); }

  async function scrapePlant(taxonid) {
    const r = await fetch('https://plantfinder.mobot.org/PlantFinderDetails.aspx?taxonid=' + taxonid + '&isprofile=0');
    const html = await r.text();
    const profileIdx = html.indexOf('Common Name:');
    if (profileIdx < 0) return null;
    const profile = html.slice(profileIdx, profileIdx + 8000);
    function field(label) {
      const re = new RegExp(label + ':\\\\s*(?:<[^>]+>\\\\s*)*([^<\\\\n]+)','i');
      const m = profile.match(re);
      return m ? ent(m[1]).replace(/\\s*(Culture|Problems|Uses|Noteworthy)/i,'').trim() : '';
    }
    function section(heading) {
      const re = new RegExp('<h[2-6][^>]*>\\\\s*' + heading + '\\\\s*<\\\\/h[2-6]>\\\\s*<p[^>]*>([\\\\s\\\\S]*?)<\\\\/p>','i');
      const m = html.match(re);
    ...
    return {
      commonName: field('Common Name'), plantType: field('Type'), family: field('Family'),
      nativeRange: field('Native Range'), zone: field('Zone'),
      heightFeet: field('Height'), spreadFeet: field('Spread'),
      bloomTime: field('Bloom Time'), bloomColor: field('Bloom Description'),
      sun: field('Sun'), water: field('Water'), maintenance: field('Maintenance'),
      attracts: field('Attracts'), tolerate: field('Tolerate'),
      culture: section('Culture'), noteworthy: section('Noteworthy Characteristics'),
      problems: section('Problems'), uses: section('Uses'),
    };
  }

  async function searchTaxonid(name) {
    const r = await fetch('/PlantFinderListResults.aspx?basic=' + encodeURIComponent(name));
    const html = await r.text();
    const m = html.match(/PlantFinderDetails\\.aspx\\?taxonid=(\\d+)/i) || html.match(/taxonid=(\\d+)/i);
    return m ? m[1] : null;
  }
  ...
  for (let i = 0; i < TAXONIDS.length; i++) {
    const id = TAXONIDS[i];
    console.log('[MBG] 처리중 (' + (i+1) + '/' + TAXONIDS.length + '): taxonid=' + id);
    try {
      const data = await scrapePlant(id);
      if (!data || !data.commonName) { console.warn('  → 데이터 없음 (MBG에 없는 식물)'); continue; }
      const pr = await fetch(WORKER + '/api/kv/set', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ key: 'mbg_detail_' + id, value: data })
      });
      const pj = await pr.json();
      console.log('  → 저장 완료:', data.commonName, pj.ok ? '✓' : '✗');
      results.push({ taxonid: id, name: data.commonName, ok: !!pj.ok });
      await new Promise(r => setTimeout(r, 500)); // rate limit
    } catch(e) { console.error('  → 오류:', e.message); }
  }
  console.log('\\n══ 완료 ══ ' + results.filter(r=>r.ok).length + '/' + TAXONIDS.length + '개 성공');
  return results;
})();
`;
